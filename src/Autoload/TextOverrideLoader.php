...
class TextOverrideLoader
{
	...
	{
		if ($className !== 'Joomla\CMS\Language\Text') {
			return false;
		}
	...
	private function ensureCachedOriginal(): void
	{
		...
		// Rename the class from Text to ViewBuilderOriginalText
		// Keep the same namespace so it remains in Joomla\CMS\Language
		$code = str_replace(
			'class Text',
			'class ViewBuilderOriginalText',
			$code
		);
